...
public class CardsController {
        ...
        @Operation(summary = "Fetch Total Loan REST API", description = "REST API to fetch card details based on a mobile number")
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "HTTP Status OK"),
                        @ApiResponse(responseCode = "500", description = "HTTP Status Internal Server Error", content = @Content(schema = @Schema(implementation = ErrorResponseDto.class)))
        })

        // Controller에 /fetchTotalLoan API 생성
        @GetMapping("/fetchTotalLoan")
        public ResponseEntity<LoansDto> fetchTotalLoan(
                        @RequestParam @Pattern(regexp = "(^$|[0-9]{10})", message = "Mobile number must be 10 digits") String mobileNumber) {
