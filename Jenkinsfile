pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = "group1-team6-cha-loan"
        IMAGE_TAG = "v0.1"
    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                git branch: 'main', url: 'https://github.com/chayeonhee/loans.git'
            }
        }
        ...
        stage('Deployment Image to Update') {
            steps {
                script {
                    ...
                    sh "kubectl set image deployment/loan-cha loan-cha=${REGISTRY}/${IMAGE_NAME}:${IMAGE_TAG} --namespace=${NAMESPACE}"
                }
            }
        }
    }
